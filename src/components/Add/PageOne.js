...
const PageOne = ({ confirmSave, authState }) => {
    ...
    const [insert_menu, { loading: menuInsertLoading }] = useMutation(
        INSERT_MENU_ITEM
    );
    ...
    const handleSubmit = async e => {
        ...
        let updatedShared =
            (katie.menu_id && !chris.menu_id) ||
            (!katie.menu_id && chris.menu_id)
                ? false
                : shared;

        let splitIncidentals = !incidentals
            ? 0
            : katie.menu_id && chris.menu_id
            ? incidentals / 2
            : incidentals;
        ...
        if (output.length) {
            try {
                const res = await insert_meals({
                    variables: { data: output },
                    refetchQueries: [{ query: GET_ALL_MEALS }],
                });
    ...
    const menuCategories = [
        ...new Set(menu.map(menu_item => menu_item.category)),
    ].sort();
    ...
    return (
        <Row>
            <Col>
                ...
                {!authState && (
                    <Text type="warning">
                        Please log in to make any changes
                    </Text>
                )}
                ...
                <Form onSubmit={handleSubmit} labelAlign="left">
                    ...
                    <Form.Item style={{ marginTop: "2em" }}>
                        <Switch
                            ...
                            onChange={e =>
                                setFormData({ ...formData, testing: e })
                            }
                        />
                    </Form.Item>
                </Form>
                ...
                <Form onSubmit={e => handleAddMenu(e)}>
                    <Form.Item label="Category">
                        <AutoComplete
                            ...
                            onChange={val =>
                                setModalData({ ...modalData, category: val })
                            }
                    ...
                    {modalError && (
                        <Row>
                            <Text type="danger">{modalError.message}</Text>
                        </Row>
                    )}
                </Form>
            </Modal>
